...
from ietf.codestands.matches.forms import SearchForm, ProjectContainerForm, ContactForm
from ietf.codestands.requests.forms import CodeRequestForm, DocNameForm, TagForm, MentorForm
from ietf.codestands.matches.models import ProjectContainer, ProjectTag, ProjectContact
...
def show_list(request, type_list="all", att=constants.ATT_CREATION_DATE, state="False", page=1):
    ...
    return render_page(request, constants.TEMPLATE_REQUESTS_LIST, {
        ...
        'typelist': type_list,
        'state': state,
        'template': 'ietf.codestands.requests.views.show_list'
    })
...
def search(request, type_list="all"):
    ...
    if search_type:
        ...
        return HttpResponseRedirect(
            settings.CODESTANDS_PREFIX + '/codestands/requests/show_list/' + 
...
def show(request, pk):
    """ Show individual Codestands Project
        :param request: HttpResponse
...
def save_project(request, template, project_container=None):
    ...
    previous_template = "codestands/requests/show_list"
    ...
    if request.method == 'POST':
        ...
        # Saving project (new or not) in the database
            ...
            return HttpResponseRedirect(settings.CODESTANDS_PREFIX + '/codestands/requests/' + str(project.id))
