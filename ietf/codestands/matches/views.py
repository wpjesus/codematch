...
from ietf.codestands.matches.forms import SearchForm, ProjectContainerForm, CodingProjectForm, LinkImplementationForm
from ietf.codestands.requests.forms import TagForm, DocNameForm
from ietf.codestands.matches.models import ProjectContainer, CodingProject, Implementation, ProjectTag
from ietf.codestands.helpers.utils import (render_page, is_user_allowed, clear_session, get_user)
from django.conf import settings
from django.core.paginator import Paginator


def show_list(request, is_my_list="False", att=constants.ATT_CREATION_DATE, state="False", page=1):
    """ List all CodeStands type_list (all = All CodeRequests / mylist = CodeRequests I've Created /
    ...
    return render_page(request, constants.TEMPLATE_MATCHES_LIST, {
        ...
        'mylist': is_my_list,
        'state': state,
        'template': 'ietf.codestands.matches.views.show_list'  # TODO fix this
    })


def show(request, pk, ck):
    """ Show individual Codestands Project and Add Implementation
        :param request: HttpResponse
...
def search(request, is_my_list="False"):
    """ Shows the list of CodeStands, filtering according to the selected checkboxes
        :param request: HttpResponse
    ...
    if search_type:
        ...
        return HttpResponseRedirect(
            settings.CODESTANDS_PREFIX + '/codestands/matches/show_list/' + 
...
def save_code(request, template, pk, ck="", coding=None):
    ...
    previous_template = "codestands/matches/show_list"  # Fix this
    ...
    if request.method == 'POST':
        ...
        # Saving project (new or not) in the database
            ...
            return HttpResponseRedirect(
                settings.CODESTANDS_PREFIX + "/codestands/matches/" + str(project.id) + '/' + str(coding_project.id))
...
@login_required(login_url=settings.CODESTANDS_PREFIX + constants.TEMPLATE_LOGIN)
...
def new(request, pk=""):
    ...
    # User must have permission to add new CodeStands
...
def remove_link(request, ck, link_name):
    """ Adds the removal list, but will only be removed when saving changes
        ck (ck = 0 - new CodeStands / ck > 0 edit CodeStands
        :param request: HttpResponse
        :param ck: int - Indicates which coding must be loaded
